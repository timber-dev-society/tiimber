...
We need a to print `Hello world` into the `content` outlet define into the `DefaultLayout` and we need to do that when a `request` is received on `hello` route defined into `routes.json`.
